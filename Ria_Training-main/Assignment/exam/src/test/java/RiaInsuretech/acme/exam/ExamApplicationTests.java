...
class ExamApplicationTests {
	...
	@Test
	void NotAuthorisedTest() throws Exception {
	...
	public void testUser() throws Exception {
		...
		MvcResult tResponse = mvc
				...
				.andExpect(status().isOk()).andExpect(jsonPath("$.jwt", Matchers.notNullValue())).andReturn();// .contentType(org.springframework.http.MediaType.APPLICATION_JSON_UTF8)

		String token = "Bearer "
				+ mapper.readValue(tResponse.getResponse().getContentAsString(), AuthenticationResponse.class).getJwt();

		MvcResult ques = mvc.perform(MockMvcRequestBuilders.get("/Exam/Start").header(HttpHeaders.AUTHORIZATION, token)
				.param("subject", "Math1")).andExpect(status().isOk()).andReturn();
		String id = mapper.readValue(ques.getResponse().getContentAsString(), Question.class).getId();
		assertEquals("1.0", id);

		/// Testing the GetNextQuestion
		ques = mvc.perform(MockMvcRequestBuilders.get("/Exam/nextQuestion").header(HttpHeaders.AUTHORIZATION, token)
				.param("ans", "a").param("subject", "Math1")).andExpect(status().isOk()).andReturn();
		id = mapper.readValue(ques.getResponse().getContentAsString(), Question.class).getId();
		assertEquals("1.1", id);
		...
		assertEquals("2.1", id);
		ques = mvc.perform(MockMvcRequestBuilders.get("/Exam/nextQuestion").header(HttpHeaders.AUTHORIZATION, token)
				.param("ans", "c").param("subject", "Math1")).andExpect(status().isOk()).andReturn();
		id = mapper.readValue(ques.getResponse().getContentAsString(), Question.class).getId();
		...
		MvcResult res = mvc.perform(MockMvcRequestBuilders.get("/Exam/Score").header(HttpHeaders.AUTHORIZATION, token)
				.param("subject", "Math1")).andExpect(status().isOk()).andReturn();

		String score = mapper.readValue(res.getResponse().getContentAsString(), String.class);
